...
photo = ['calc.ico', 'eap_logo2.png', 'smartcalc.jpg', 'serafina.jpg', 'xenakis.jpg', 'microphone.png']
...
def terminate(event):
    ...
    verification = messagebox.askquestion("Τερματισμός προγράμματος", "Θέλετε σίγουρα να τερματίσετε το πρόγραμμα;")
    if verification == 'yes':
        messagebox.showinfo("Επιβεβαίωση τερματισμού", "Ευχαριστούμε που χρησιμοποιήσατε το Smart! Calc\n\n [ΠΛΗΠΡΟ - ΗΛΕ52] 2023 - 2024\nΟμάδα_5_Project\n\nΠιέστε ΟΚ για τερματισμό.")
...
να την καλέσει, τότε θα παραχθεί σφάλμα'''
    ...
    global clock
    clock = Label(start_window, text = display_new_time, font = ('Helventica', 8, 'bold'), fg='#000000', bg='#e1e1ea')
    clock.grid(row = 0, column = 17, columnspan = 3, sticky = W + E)
...
##    global total_memory
##    if len(total_memory) > 0: clear_all()
    
    #περιεχόμενα οθόνης
    user_input = screen.get()
    
    #έλεγχος πλήθους ψηφίων (αποδεκτά: <= 20)
    if len(user_input) <= 20:        
        #έλεγχος αν το μηδέν είναι πρώτο ψηφίο και ΔΕΝ ακολούθησε υποδιαστολή
        if user_input != '0':

#καθαρισμός της οθόνης όταν οι πράξεις συνεχίζονται με μνήμη
    ########if len(user_input) > 0 and user_input[0] == 'M': clear_all()
            ...
            #if memory_on: digit += u'\u1D39'
            #εισοδος των ψηφίων στα δεξιά της οθόνης
##            clear_all()
##            screen.insert(0, digit)
            show_to_screen(digit)
...
def dot():
    ...
    if previous_dot == -1: allow_dot = True
        ...
        for character in user_input[-1 : - previous_dot : -1]:
            if character in ['+', '-', '*', '/', '**']:
                allow_dot = True
                break
...
def reverse_operator():
    ...
    if digit[-1] == ')':
        ...
        if start > 0: operator = digit[start - 1]
        else: operator = ''        
        #...και αντιστροφή του
        #match case
        match(operator):
...
def what_to_do(number):
    ...
    match(number):
        ...
        case 5:
            ...
            clear_all()
            screen.insert(0, str(digit) + ' M')
            total_memory = ''
        #MC (καθαρισμός μνήμης -> user-defined function)
        case 6:            
            digit = total_memory[:-1]            
            show_to_screen(digit)
            memory_on = False            
...
def make_main_frame():
    ...
    #header
    headerTitle = Label(start_window, text = "Smart Calc", font = ("Helvetica", 40, "bold"), bg = bg_color, fg = "#000000", padx = 220, anchor = 'center')
    headerTitle.grid(row = 0, column = 5, columnspan = 10, sticky =  E + W)
    headerSubTitle = Label(start_window, text = "Scientific Calculator", font = ("Helvetica", 11, "bold"), bg=bg_color, fg = "#000000", padx = 0)
    headerSubTitle.grid(row = 1, column = 0, columnspan = 20, sticky =  E + W)

    #χρησιμοποιώ κενά Labels για να με βοηθήσουν στη διαταξη (καλύτερη στοιχιση)
    blank_label1 = Label(start_window, text = "", font = ("Helvetica", 40, "bold"), bg = bg_color, fg = "#000000", padx = 28, anchor = 'center')
    blank_label1.grid(row = 0, column = 1, columnspan = 4, sticky =  E + W, padx = 65)
    ...
    menuBtknsList = ["Home", "About", "Calculator", "Info"]    
    ...
    for i in range(len(menuBtknsList)):
        menuBtn = Button(buttonsFrame, text = menuBtknsList[i], fg = bg_color, bg = "#8787ab", width = 20, pady = 15, font = ('Helvetica', 9, 'bold'), anchor = 'center', command = lambda i = i: change(frames_list[i]))
        menuBtn.grid(row = 4, column = 3 + 4 * i, columnspan = 2, padx = 30)
        
    introLabel0 = Label(buttonsFrame, text = "[ΠΛΗΠΡΟ - ΗΛΕ52] 2023 - 2024\nΟμάδα_5_Project", font = ('Helvetica', 10, 'bold'), bg = bg_color, fg = "#000000", anchor = 'center', pady = 20)
    introLabel0.grid(row = 10, column = 5, columnspan = 10, sticky =  E + W)
    introLabel1 = Label(buttonsFrame, text = "Τίτλος Project: Ανάπτυξη scientific calculator με τη βιβλιοθήκη tkinter", font = ('Helvetica', 8, 'bold'), bg = bg_color, fg = "#000000", anchor = 'center', pady = 20)
    introLabel1.grid(row = 10, column = 5, columnspan = 10, sticky =  E + W)
    
    #εικόνα κεντρικής οθόνης
    calc_label.grid(row = 20, column = 5)#, columnspan = 5)
...
def make_calc_frame():
    ...
    for i in range(len(buttons_names_list)):
        ...
        if i in [15, 16, 17, 25, 26, 27, 35, 36, 37, 38]:
            lambda i = i: calc_btn.bind(str(numbers_dict[str(i)]), what_to_do(i))                
...
def make_about_frame():
    '''
        Διαδικασία (οδηγίες σχετικά με το scientific calculator).
    '''
    ...
    about_label = Label(AboutFrame, text = str(messages_dict['gr'][0]).center(0), fg = "#000000", bg = bg_color)
    about_label.grid(row = 10, column = 5, columnspan = 10)
...
def make_info_frame():
    ...
    global InfroFrame
    section = "InfoFrame"    
    InfoFrame.grid(row = 5, column = 0, columnspan = 20, sticky =  N + S + E + W)
    blank_label = Label(InfoFrame, text = "", fg = "#000000", padx = 200, bg = bg_color)
    blank_label.grid(row = 10, column = 0)    
    about_label1 = Label(InfoFrame, text = str(messages_dict['gr'][1][:str(messages_dict['gr'][1]).index("Τίτλος")]), font = ('Helvetica', 10, 'bold'), fg = "#000000", bg = bg_color)
    about_label1.grid(row = 10, column = 5, columnspan = 10)
    about_label2 = Label(InfoFrame, text = str(messages_dict['gr'][1][str(messages_dict['gr'][1]).index("Τίτλος") :]), fg = "#000000", bg = bg_color)
    about_label2.grid(row = 11, column = 5, columnspan = 10)
...
def main():    
    ...
    #τίτλος
    start_window.title("[ΠΛΗΠΡΟ - ΗΛΕ52] 2023 - 2024 Ομάδα_5_Project    SmartCalc v.1.0.4")
    
    #εικονίδιο (πάνω αριστερά)
    #start_window.iconbitmap("calc.ico")
    ...
    #start_window.state('zoomed')  #μεγέθυνση του παραθύρου στο μέγεθος της οθόνης
    ...
    MainFrame.grid(row = 5, column = 0, columnspan = 20, sticky =  N + S + E + W)
    imageFrame = Frame(MainFrame, bg = bg_color, pady = 30, padx = 436)
    imageFrame.grid(row = 10, column = 0, columnspan = 20, sticky = E + W, pady = 10)
    ...
    #microphone.grid(row = 7, column = 17, padx = 5, sticky = E + N)
    #microphone.config(bg = colors[theme][0][1])
    #microphone_label = Label(CalcFrame, image = microphone_img, bg = colors[theme][0][1])
