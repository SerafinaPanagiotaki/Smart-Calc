...
#βοηθητική μεταβλητή για να παίξει sound (έλεγχος από τον χρήστη)
start = True
...
ή πιέστε <Esc> για έξοδο.''', 'Τεκμηρίωση', "ΕΑΠ [ΠΛΗΠΡΟ - ΗΛΕ52] 2023 - 2024 Ομάδα_5_Project    Smart Calc v.3.1", ('Σφάλμα:', 'Δε βρέθηκε το αρχείο τεκμηρίωσης.')],
...
or press <Esc> to exit.''', 'Documentation', "HOU [ΠΛΗΠΡΟ - ΗΛΕ52] 2023 - 2024 Team_5_Project    Smart Calc v.3.1", ('Error:', 'Documentation file not found.')]                      
                }
...
Διαθέσιμες γλώσσες σε αυτή την έκδοση: 1. Ελληνικά 2. Αγγλικά'''
    ...
    try:
        ...
        documentation_label.config(text = messages_dict[str(lang)][7])
    except: pass
    try:
        global documentation_btn
        documentation_btn.config(text = messages_dict[str(lang)][8])
    except: pass
    
    #τίτλος (χωρίς try... except, καθώς σε κάθε αλλαγή γλώσσας αλλάζει ο τίτλος
    #της εφαρμογής)
    start_window.title(str(messages_dict[str(lang)][9]))
    ...
    match(number):
        ...
        case 28:            
            ...
            try:
                digit = eval(user_input)                
                if len(str(digit)) > 20:
                    if digit < sys.float_info.min or digit > sys.float_info.max: show_to_screen('Error')
                    else:
                        try:
                            digit = '%e'%float(digit)
                            show_to_screen(digit)
...
def key_command(event):
    '''
        Συνάρτηση που ανάλογα με το πλήκτρο που πατήθηκε καλεί την what_to_do(i).
    '''
    #print(key_command.__doc__)

    #έλεγχος του πλήκτρου που πατήθηκε    
...
def make_main_frame():
    ...
    global speaker_off
    global speaker_btn
    speaker_btn = Button(buttonsFrame, image = speaker_off, bg = bg_color, relief = FLAT, activebackground = bg_color, command = play_music) 
...
def make_info_frame():
    ...
    documentation_label = Label(InfoFrame, text = str(messages_dict[str(lang)][7]), fg = "#000000", bg = bg_color, justify = LEFT)
    documentation_label.grid(row = 25, column = 5, columnspan = 10, pady = 20)
    documentation_btn = Button(InfoFrame, text = str(messages_dict[str(lang)][8]), fg = bg_color, bg = "#8787ab", justify = LEFT, font = ('Helvetica', 9, 'bold'), anchor = 'center',  width = 20, pady = 15, command = open_documentation)
    documentation_btn.grid(row = 26, column = 5, columnspan = 10, pady = 5)
...
def main():    
    ...
    #keyboard binding
    start_window.bind("<Key>", key_command)
